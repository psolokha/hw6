...
import { addFavorite, listFavorites, removeFavorite, syncFavorites } from "../../db/favorites.repo.js";
...
export async function registerFavoritesRoutes(app: FastifyInstance) {
  app.get("/api/favorites", async (req, reply) => {
    ...
    if (!userId) return reply.status(401).send({ error: { message: "Не авторизован", kind: "UNKNOWN" } });
  ...
  app.post("/api/favorites", async (req, reply) => {
    ...
    if (!parsed.success) return reply.status(400).send(validationError(zodErrorMessage(parsed.error)));
